...
export default function AdminUserApprovalPage() {
    ...
    // Fetch pending users using the CSRF token returned from fetchCsrfToken
    const fetchPendingUsers = useCallback(async () => {
        setLoading(true);
        setError(null);
        setMessage(null);
        ...
        try {
            const token = await fetchCsrfToken(); // Always get fresh token
            if (!token) {
    ...
    // Initialize fetching once when user is authenticated
    useEffect(() => {
        if (isAuthenticated && !hasFetchedRef.current) {
            fetchPendingUsers();
            hasFetchedRef.current = true;
