...
export default function AdminUserApprovalPage() {
    ...
    const fetchPendingUsers = async () => {
        ...
        try {
            ...
            const response = await fetch('http://hr-backend-xs34.onrender.com/api/admin/users/pending-approval/', {
                method: 'GET',
                headers: {
    ...
    const handleApproveUser = async (userId) => {
        ...
        try {
            const csrfToken = getCookie('csrftoken');
            const response = await fetch(`http://hr-backend-xs34.onrender.com/api/admin/users/${userId}/approve/`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRFToken': csrfToken,
