...
;
    ...
    useEffect(() => {
        ...
        const fetchAssessment = async () => {
            ...
            try {
                ...
                const response = await fetch(`http://hr-backend-xs34.onrender.com/api/employee/self-assessments/${effectiveId}/`, {
                    method: 'GET',
                    headers: {
        ...
        fetch('http://hr-backend-xs34.onrender.com/api/csrf/', { credentials: 'include' });
    ...
    useEffect(() => {
        ...
        if (!isEditMode && formData.quarter_number && formData.year.length === 4 && user?.id) {
            const check = async () => {
                ...
                try {
                    const csrfToken = getCookie('csrftoken');
                    const res = await fetch(`http://hr-backend-xs34.onrender.com/api/employee/self-assessments/?quarter_number=${formData.quarter_number}&year=${formData.year}`, {
                        method: 'GET',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLoading(true);
            const csrftoken = getCookie('csrftoken');
            let url = 'http://hr-backend-xs34.onrender.com/api/employee/self-assessments/';
            let method = 'POST';

            if (isEditMode) {
                url = `http://hr-backend-xs34.onrender.com/api/employee/self-assessments/${effectiveId}/`;
                method = 'PATCH';
            }
