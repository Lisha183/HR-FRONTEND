...
//         try {
//             const csrftoken = getCookie('csrftoken');
//             const response = await fetch('https://hr-backend-xs34.onrender.comapi/admin/self-assessments/?status=Pending HR Review', {
//                 method: 'GET',
//                 headers: {
//                     'Content-Type': 'application/json',
//                     'X-CSRFToken': csrftoken,
...
export default function AdminMeetingSlotManagement() {
    ...
    const fetchSelfAssessmentsForDropdown = async () => {
        try {
            const tokenToUse = await getToken();
            const response = await fetch('https://hr-backend-xs34.onrender.com/api/admin/self-assessments/?status=Pending HR Review', {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRFToken': tokenToUse,
                            ...
                            {[...meetingSlots]
                                ...
                                .map(slot => (
                                    <tr key={slot.id}>
                                        ...
                                        <td>{slot.end_time}</td>
                                        <td>{slot.is_booked ? 'Yes' : 'No'}</td>
                                        <td>{slot.booked_by_employee_username || 'N/A'}</td>
                                        <td>
                                            <button onClick={() => setEditingSlot({ ...slot })} className="approve-button mr-2">
                                                Edit
                                            </button>
